...
window.addEventListener(
  ...
  function() {
    ...
    let skyParticleSystem = new THREE.Points(skyParticles, skyParticlesMaterial)
    // add it to the scene
      ...
      if (smokeParticles != null) {
        for (let i = 0; i < smokeParticleCount; i++) {
          if (smokeParticles.vertices[i].distanceTo(planeGroup.position) < s) {
            ...
            smokeParticles.vertices[i].x =
              smokeEmitter.x + Math.random() * 3.5 - 1.75
            smokeParticles.vertices[i].y =
              smokeEmitter.y + Math.random() * 3.5 - 1.75
            smokeParticles.vertices[i].z =
              smokeEmitter.z + Math.random() * 3.5 - 1.75
          }
        }
